...
router.get("/", async (req, res) => {
  ...
  const teachers = conditions.length > 0 ? await db.select().from(studentsTable).where(and(...conditions)) : await db.select().from(studentsTable);
  res.json(teachers.map((t) => ({ ...t, email: t.email ?? undefined, phone: t.phone ?? undefined, remarks: t.remarks ?? undefined })));
});
...
router.post("/", async (req, res) => {
  ...
  const now = new Date();
  const autoTime = now.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: true, timeZone: "Asia/Manila" });
  ...
  const timeValue = providedTime && providedTime.length > 0 ? providedTime : chosenStatus === "PRESENT" ? autoTime : "N/A";
  const [created] = await db.insert(studentsTable).values({
    name: parsed.data.name,
    email: parsed.data.email ?? null,
    phone: parsed.data.phone ?? null,
    block: parsed.data.block ?? "TBD",
    course: parsed.data.subject ?? parsed.data.course ?? "TBD",
  ...
  res.status(201).json({ ...created, email: created.email ?? undefined, phone: created.phone ?? undefined, remarks: created.remarks ?? undefined });
});
...
router.get("/stats/summary", async (_req, res) => {
  ...
  res.json({ total: result.total ?? 0, present: result.present ?? 0, absent: result.absent ?? 0, late: result.late ?? 0 });
});
...
router.get("/:id", async (req, res) => {
  ...
  res.json({ ...teacher, email: teacher.email ?? undefined, phone: teacher.phone ?? undefined, remarks: teacher.remarks ?? undefined });
});
...
router.put("/:id", async (req, res) => {
  ...
  if (data.subject !== undefined) updateValues.course = data.subject;
  ...
  const [updated] = await db.update(studentsTable).set(updateValues).where(eq(studentsTable.id, paramsParsed.data.id)).returning();
  res.json({ ...updated, email: updated.email ?? undefined, phone: updated.phone ?? undefined, remarks: updated.remarks ?? undefined });
});
